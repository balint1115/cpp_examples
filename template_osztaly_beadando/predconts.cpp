...
// template osztály, ahol az első template paraméter típusának megfelelő két
// szekvenciális konténerbe egy predikátum (aminek típusa a második template
// paraméter) szerint tároljuk el az elemeket. Az utolsó néhány ellenőrzésnél
// a konténerben tárolt típus nincs template paraméterként megadva.
...
// az elem a predikátum szerint igaz, akkor az első konténerbe, különben a
// másodikba kerül, tetszőleges pozicióra. A konstruktorban a predikátum
// objektumot is megadom, hogy állapottal rendelkező funktorokat is lehessen
...
// előfordulását törli. Az intervallumot váró insert az intervallum összes
// elemét hozzáadja a megfelelő konténerhez.
...
template<typename T, class U, class V = void>
...
public:
    ...
    predicate_containers(T& c1, T& c2, U predicate) : predicate(predicate)
    {
        container1 = &c1;
        container2 = &c2;
        container1->clear();
    ...
    predicate_containers(T& c1, T& c2)
    {
        container1 = &c1;
        container2 = &c2;
        container1->clear();
    ...
    predicate_containers insert(typename T::value_type element)
    {
        if (predicate(element))
        {
            container1->push_back(element);
        }
        else
        {
            container2->push_back(element);
        }

        return *this;
    }

    predicate_containers insert(typename T::value_type *begin, typename T::value_type *end)
    {
        for (typename T::value_type *it=begin; it != end; it++ )
        {
            insert(*it);
        }
        return *this;
    ...
    predicate_containers insert(typename T::iterator begin, typename T::iterator end)
    {
        for (; begin != end; begin++ )
        {
            insert(*begin);
    ...
    predicate_containers erase(typename T::value_type element)
    {
        if (predicate(element)){
            for (typename T::iterator it = container1->begin(); it != container1->end(); it++)
            {
                if (*it == element)
                {
                    container1->erase(it--);
        ...
        {
            for (typename T::iterator it = container2->begin(); it != container2->end(); it++)
            {
                if (*it == element)
                {
                    container2->erase(it--);
